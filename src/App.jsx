      ...
      if (textRef.current) {
        ...
        for (let i = 0; i < textNodes.length; i++) {
          ...
          const distanceFromTop = document
            .querySelector(".textreveal_text__9qSpD")
            .getBoundingClientRect().top;
          console.log(window.innerHeight, distanceFromTop);
          if (
            Math.abs(window.innerHeight + distanceFromTop) <=
            window.innerHeight + 520 - (i + 1) * 20
          )
            textNodes[i].style.opacity = 1;
          if (
            Math.abs(window.innerHeight + distanceFromTop) >=
            window.innerHeight + 520 - (i + 1) * 20
          )
            textNodes[i].style.opacity = 0;
        }
      }
    }
              ...
              <div class="textreveal_wrapper__fuEcC">
                <div class="textreveal_sticky__NBmkY">
                  <div class="textreveal_inner__Kddvp">
                    <div class="relative">
                      <p
                        class="text-white title-lead font-semibold leading-none absolute"
                        ref={textRef}
                      >
                      ...
                      <p class="title-lead font-semibold leading-none text-[#202020]">
                        In Chronicle everything is made with Blocks that come
                        with pixel perfect design, interactivity and motion out
                        of the box. Instead of designing from scratch, simply
                        choose the right one from our library of blocks and see
                        the magic unfold.
                      </p>
                    </div>
                  </div>
